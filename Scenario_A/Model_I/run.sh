...
if [ ! -f resultsNumerical.txt ]; then
	...
	"Beta" "Phi" "S" "Cost" "SegregationLoss" "Alpha" "MutProb" "N0" "S0" "TimeMaxIntegrand" "ResolutionIntegrand" "pRNumerical"
	EOF
fi

if [ ! -f resultsSimulation.txt ]; then
	cat > resultsSimulation.txt <<-EOF
	"Beta" "Phi" "S" "Cost" "SegregationLoss" "Alpha" "MutProb" "N0" "S0" "NSim" "pRSimulation"
	EOF
fi
