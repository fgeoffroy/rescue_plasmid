...
bool rescue(double beta, double phi, double sM, double c, double alpha,
  ...
  double a[6]; // declare array for propensity values.
  ...
  while ((N!=0) && Nvec[1]<= log (.01) / log((1 + c) / (1 + sM)) ) {
...
int main(int argc, char const *argv[]) {
  ...
  for (size_t k = 0; k < NSim; k++) {
    bool b=rescue(i.beta, i.phi, i.sM, i.c, i.alpha, i.u, (int)i.N0, i.s0, k);
    result+= b;
  }
  result/=NSim;

  std::ofstream log("resultsSimulation.txt", std::ios_base::app | std::ios_base::out);
  log << i.beta << " " << i.phi << " " << i.sM << " " << i.c << " " << i.alpha << " " << i.u << " " << i.N0 << " " << i.s0 << " " << NSim << " " << result << "\n";
