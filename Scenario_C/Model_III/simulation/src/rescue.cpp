...
bool rescue(double beta, double gamma, double sP, double sM, double c,
  double sigma, double alpha, double u, double s0, unsigned int seed) {
  ...
  if (beta <= beta_free) {
    ...
    if (sigma == 0) {
      ...
      if (beta >= beta_wild) {
        Nvec[0] = 0;
        Nvec[1] = round((sP - c) / alpha);
      } else {
        Nvec[0] = round((alpha * c - beta * (sP - c)) / (beta * beta));
        Nvec[1] = round((beta * sP - alpha * c) / (beta * beta));
      }
    }
    else {
      Nvec[0] = round((alpha*c + sqrt(c*(pow(alpha + beta, 2)*c + 4*alpha*beta*sigma) - 2*beta*c*(alpha + beta - 2*alpha*sigma)*sP + pow(beta*sP,2)) + beta*(c - sP + 2*sigma*(1 + sP)))/(2.*pow(beta,2)));
      Nvec[1] = round(-(pow(alpha,2)*c + alpha*(-(beta*sP) + 2*beta*sigma*(1 + sP) + sqrt(pow(alpha*c,2) + pow(beta*(c-sP),2) + 2*alpha*beta*c*(c - sP + 2*sigma*(1 + sP)))) - beta*(beta*(-c + sP) + sqrt(pow(alpha*c,2) + pow(beta*(c-sP),2) + 2*alpha*beta*c*(c - sP + 2*sigma*(1 + sP)))))/(2.*alpha*pow(beta,2)));
    }
  }
  ...
  double a[11]; // declare array for propensity values.
  ...
  while ((N!=0) && Nvec[2]<= log (.01) / log((1 + c) / ((1 + sM) * (1 - sigma))) ) {
    ...
    a[8] = betaM * Nvec[1] * Nvec[2];
    /** Immigration event of a not incompatible resistant plasmid via HT (we assume no effect of beta here) **/
    a[9] = u * Nvec[0];
    a[10] = u * Nvec[1];
    ...
    if (r<3) { // birth
      ...
      if (r == 7 || r == 9) { // transfer to a plasmid-free cell
        Nvec[0]--;
      } else { // transfer to a wildtype plasmid-bearing cell
        Nvec[1]--;
      }
      Nvec[2]++;
    }
...
int main(int argc, char const *argv[]) {
  ...
  for (size_t k = 0; k < NSim; k++) {
    bool b=rescue(i.beta, i.gamma, i.sP, i.sM, i.c, i.sigma, i.alpha, i.u, i.s0, k);
    result+= b;
  }
  result/=NSim;

  std::ofstream log("resultsSimulation.txt", std::ios_base::app | std::ios_base::out);
  log << i.beta << " " << i.gamma << " " << i.sP << " " << i.sM << " " << i.c << " " << i.sigma << " " << i.alpha << " " << i.u << " " << i.s0 << " " << NSim << " " << result << "\n";
