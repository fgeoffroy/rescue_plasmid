...
if [ ! -f resultsNumerical.txt ]; then
	...
	"Beta" "Gamma" "SP" "SM" "Cost" "SegregationLoss" "Alpha" "MutProb" "S0" "TimeMaxIntegrand" "ResolutionIntegrand" "pRNumerical"
	EOF
fi

if [ ! -f resultsSimulation.txt ]; then
	cat > resultsSimulation.txt <<-EOF
	"Beta" "Gamma" "SP" "SM" "Cost" "SegregationLoss" "Alpha" "MutProb" "S0" "NSim" "pRSimulation"
	EOF
fi
